...
func TestClientHeal(t *testing.T) {
	...
	nsc := client.NewClient(ctx,
		serverURL,
		client.WithDialOptions(sandbox.DefaultDialOptions(sandbox.GenerateTestToken(clockTime))...),
...
func startEmptyServer(ctx context.Context, t *testing.T, serverURL *url.URL) context.CancelFunc {
	...
	nse := endpoint.NewServer(serverCtx, sandbox.GenerateTestToken(clockTime))
